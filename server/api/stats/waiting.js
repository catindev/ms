...
function filterSuccessCalls( calls ) {
    const filtered = calls.filter( call => call.maxDuration.waiting );
    return filtered.length;
}

function filterBadCalls( calls ) {
    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            function findCalls(error, calls) {
                ...
                resolve([
                    interval.name,
                    filterSuccessCalls( calls ),
                    filterBadCalls( calls )
                ]);
            }
        } catch( error ) {
            reject( error );
...
function barChart({ start, end, interval, account }) {
    ...
    return Promise.all( pipeline )
        .then( results => {
            results.unshift([ getTitle(interval), 'Отвеченные вовремя', 'Ожидавшие' ]);
            return results;
        })
        .catch( errorHandler );
}
