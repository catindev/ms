...
module.exports = (request, response) => {
    ...
    const render = fields => response.render(
        'reports/fields', {
            customer: report.customer, fields, range, reportID: request.params.id
        }
    );

    // const render = data => response.json(data);

    calculate(report.fields, report.accounts, report.date)
        .then( render )
        .catch( error => {
